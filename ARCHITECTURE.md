...
| Lightning evidence map | `components/LightningMap.tsx`, `components/lightning-map/`, `lib/lightning-map.ts`, `lib/slippy-map.ts` | Pure qualification/projection geometry lives in `lib/`; hooks own both read-only query layers, caching, canvas drawing/hit-test coordinates, and pointer/pinch/wheel navigation; dedicated header and evidence-footer components own presentation; the composition root owns viewport state and wiring. |
...
5. **Complete:** qualification/projection geometry is pure and tested;
   detection loading/caching, imperative canvas drawing, gesture state, map
   header, and evidence footer live behind focused modules. `LightningMap.tsx`
   remains the explicit composition root for viewport state and wiring.
