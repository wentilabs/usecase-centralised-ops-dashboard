...
export const openapiDocument = {
  ...
  info: {
    ...
    description: [
      ...
      "",
      "`getSchema` also carries two things that answer \"what does this column actually do\": `routes`, the",
      "`METHOD /path` endpoints on the service that read it, and `hasPreview`, set when HALO can show the",
      "real message that column produces.",
      "",
  ...
  paths: {
    "/api/metrics": {
      get: {
        ...
        tags: ["discovery"],
        summary: "Where this instance spent its time",
        description:
          "One row per instrumented operation, costliest first by total time, with a call count, mean, p50, p95, max and tags for cache hits, errors and timeouts. Read this before optimising: it says which operation is expensive and how often it runs, rather than how long one page took once. In-process and reset by a restart, so it ranks operations against each other rather than describing the estate.",
        responses: {
          "200": {
            description: "Timing for this process since it started.",
            content: {
              "application/json": {
                schema: {
                  type: "object",
                  properties: {
                    uptimeSeconds: { type: "integer", description: "The denominator for every count below." },
                    totalMs: { type: "integer", description: "Milliseconds recorded across every operation." },
                    operations: {
                      type: "array",
                      items: {
                        type: "object",
                        properties: {
                          name: { type: "string", description: "e.g. `supabase GET wbgts`, `model call`, `job noise-bootstrap`." },
                          count: { type: "integer" },
                          total: { type: "integer" },
                          mean: { type: "integer" },
                          p50: { type: "integer" },
                          p95: { type: "integer" },
                          max: { type: "integer" },
                          tags: { type: "object", description: "Counts that are not durations: hit, miss, ok, timeout, http_404." },
                        },
                      },
                    },
                  },
          ...
          "401": { description: "No credential, or one that is not recognised." },
          "403": { description: "The caller is not on the allow-list." },
        },
      },
    },
    "/api/session": {
                        ...
                        services: {
                          ...
                          items: {
                            ...
                            properties: {
                              ...
                              ready: {
                                type: "array",
                                description: "Creatable as they stand. Send each `values` to createProject as the `draft`, unchanged — it may carry any column of the table, and `derived` says where each non-obvious value came from.",
                                items: {
                                  type: "object",
                                  properties: {
    ...
    "/api/noise-limits": {
      get: {
        ...
        description: [
          ...
          "",
          "Stored per HOUR and returned per BAND: the source page presents eight uneven bands (7am-7pm, 7pm-8pm, 8pm-10pm, 10pm-12am, 12am-2am, 2am-5am, 5am-6am, 6am-7am) which the refresh expands to 48 rows per meter (24 hours x Mon-Sat / Sun-PH). Contiguous hours holding identical limits are collapsed back, but never across one of the eight boundaries, so the rows returned are the rows on the source page — ordered from 07:00, starting at 7am-7pm and ending at 6am-7am, which is that page's column order and the day boundary the service uses elsewhere. An hour that differs inside a band still splits out rather than being hidden behind whichever hour a fixed grid would sample.",
          "",
      ...
      patch: {
        operationId: "writeNoiseLimits",
        tags: ["configuration"],
        summary: "Write one meter's permissible noise levels",
        description: [
          "Changes the limits of ONE meter, band by band, and optionally protects them from the NoiseLynx refresh. Requires an editor session. The only route that writes `noise_limits`.",
          "",
          "Bands are addressed by `startMinutes`/`endMinutes` and must be bands the meter already has — this route changes existing rows and never creates a meter or a band. Each band is expanded to its hours on the way in, so `2am-5am` writes 3 rows and `7am-7pm` writes 12, and the whole set goes in ONE upsert so a save cannot land half-applied.",
          "",
          "`protect: true` writes a `source_file` carrying the marker that makes the limits refresh keep these values (`provenance` becomes the free text after it, and is the only record of why they differ from the vendor's page). `protect: false` leaves each row's existing `source_file` alone rather than clearing a marker someone else set — protection is a deliberate choice, not a consequence of editing. Values and marker always go in the same row write, so a refresh cannot land between them.",
          "",
          "`baseImportedAt` is the newest `imported_at` the caller saw, from `listNoiseLimits`. If any row has been written since, the save is refused with 409 rather than overwriting it. `noise_limits` has no `updated_at`, so this is a check-then-write rather than a true compare-and-swap.",
          "",
          "A limit of `0` or `null` means NO limit for that metric in that band, which is how the source page and the service both read it. Values outside 0-99 are accepted: the columns carry no CHECK and that is intended.",
        ].join("\n"),
        requestBody: {
          required: true,
          content: {
            "application/json": {
              schema: {
                type: "object",
                required: ["projectCode", "fullIdentifier", "bands"],
                properties: {
                  projectCode: { type: "string" },
                  fullIdentifier: { type: "string", description: "The meter, exactly as stored." },
                  bands: {
                    type: "array",
                    items: {
                      type: "object",
                      required: ["dayType", "startMinutes", "endMinutes"],
                      properties: {
                        dayType: { type: "string", enum: ["mon_sat", "sun_ph"] },
                        startMinutes: { type: "integer", minimum: 0, maximum: 1439 },
                        endMinutes: { type: "integer", minimum: 0, maximum: 1440 },
                        leq5min: { type: "number", nullable: true },
                        leq1hr: { type: "number", nullable: true },
                        leq12hr: { type: "number", nullable: true },
                      },
                    },
                  },
                  protect: { type: "boolean", description: "Exempt these rows from the limits refresh." },
                  provenance: { type: "string", description: "Where the numbers came from. Free text after the marker." },
                  baseImportedAt: { type: "string", nullable: true },
                },
              },
            },
          },
        ...
        responses: {
          "200": { description: "Rows written, and the project's limits re-read.", content: { "application/json": { schema: { type: "object", additionalProperties: true } } } },
          "400": errorResponses["400"],
          "401": errorResponses["401"],
          "403": errorResponses["403"],
          "409": { description: "A row was written after baseImportedAt." },
        },
      },
    },
    "/api/jobs/{job}": {
    ...
    "/api/onboard/{service}": {
      get: {
        ...
        description:
          "The full vocabulary of a `createProject` draft, plus the prefilled defaults the server can supply, which environment-backed defaults are missing, and whether any prerequisite database function is installed.\n\n`fields` is every column a new row can carry — the whole config table, not a shortlist. It is built from live schema introspection, so it follows the database rather than this document: read it rather than assuming. Each entry says whether the column is `required`, what `options` a select accepts, and whether it is `fromSchema` — meaning it has no curated default and, left out of the draft, simply keeps the column's database default.\n\nCall before `createProject` — WBGT cannot be onboarded at all until its readings-table function exists.",
        parameters: [{ name: "service", in: "path", required: true, schema: { type: "string", enum: [...SERVICE_KEYS] } }],
        responses: {
          "200": {
            description: "Every settable column, the defaults, and any prerequisite.",
            content: {
              "application/json": {
                schema: {
                  type: "object",
                  properties: {
                    fields: {
                      type: "array",
                      description: "Every column a draft may carry, in the order the dialog renders them.",
                      items: {
                        type: "object",
                        properties: {
                          column: { type: "string" },
                          ...
                          kind: {
                            type: "string",
                            enum: ["text", "sheet", "number", "groups", "multi", "toggle", "select", "hhmm"],
                            description:
                              "How the value is written. `toggle` takes \"true\"/\"false\"; `multi` and `groups` take a comma-separated list; `sheet` accepts a full Google Sheets URL and stores the id; `hhmm` is a 24-hour time like \"0800\". Everything is sent as a string.",
                          },
                          required: { type: "boolean", description: "The insert is refused without it." },
                          computed: { type: "boolean", description: "Derived server-side from the project code; anything you send for it is ignored." },
                          options: { type: "array", items: { type: "string" }, nullable: true, description: "The only accepted values, for a select or multi." },
                          target: { type: "string", enum: ["config", "companion"], description: "`companion` columns are written to a second table, not the config row." },
                          fromSchema: {
                            type: "boolean",
                            description:
                              "Present on the table but not part of the curated flow. Omit it and the column keeps its database default; nothing is written for it.",
                          },
                        },
                      },
                    },
                    prefill: { type: "object", additionalProperties: { type: "string" }, description: "Values the server will use if the draft omits them." },
                    missingEnvDefaults: { type: "array", items: { type: "string" }, description: "Declared env defaults that are unset, so those fields must be sent explicitly." },
                    rpc: { type: "object", nullable: true, additionalProperties: true, description: "A prerequisite database function and whether it is installed." },
                  },
                },
              },
            },
      ...
      post: {
        ...
        description: [
          ...
          "through `updateProjectConfig` once the project has been verified. Several column CHECKs are written on",
          "that assumption — issue-chaser refuses its summary and cadence flags while a row is disabled — so a",
          "draft that sets one of those will be rejected until the row is enabled.",
          "",
          "**The draft may carry any column of the service's config table.** Not a curated shortlist: whatever the",
          "editor would let you change on an existing row, you can set while creating it. `getOnboardingRequirements`",
          "returns the exact list with types and allowed values, derived from live schema introspection. A column",
          "you omit keeps its database default — omitting is how you say \"leave it alone\", and there is no way to",
          "write an explicit null.",
          "",
          "A key that is not a column of the service is **rejected**, naming the key. It is not ignored: a draft",
          "that half-applies and reports success is worse than one that fails.",
          "",
          ...
          "",
          "Row creation is recorded in the audit trail as an `insert`, alongside later updates.",
        ].join("\n"),
        parameters: [{ name: "service", in: "path", required: true, schema: { type: "string", enum: [...SERVICE_KEYS] } }],
        requestBody: {
          required: true,
              ...
              schema: {
                ...
                properties: {
                  draft: {
                    ...
                    description:
                      "Column name to value, every value a string. May carry ANY column of this service's config table — call `getOnboardingRequirements` for the list, the types and the allowed values. Omitted columns keep their database default; an unknown column is rejected rather than ignored.",
                  },
                },
                required: ["draft"],
              },
